...
import com.sxy.zongce.service.impl.PublicWelfareServiceImpl;
...
public class PublicWelfareController {
    ...
    public ResponseRes<Object> incrementImport(MultipartFile file) {
        try {
            ...
            //定义publicWelfareList变量，用来存储文件内容(违纪通报)
            List<PublicWelfare> publicWelfareList = new ArrayList<>();
            ...
            for (int i = 2; i < list.size(); i++) {
                ...
                //定义publicWelfare遍历，存储第i行数据
                PublicWelfare publicWelfare = new PublicWelfare();
                //对第i行数据进行遍历，
                for (int j = 0; j < rows.size(); j++) {
                    //获取第i行第j列数据，存到cellVal 变量里面
                    String cellVal = (String) rows.get(j);
                    //调用setFileValueByFieldName函数，把数据存到publicWelfare对应的属性里面
                    ExcelUtil.setFileValueByFieldName(publicWelfare, firstRows.get(j).toString().trim(), cellVal);
                }
                //publicWelfare变量加到publicWelfareList
                publicWelfareList.add(publicWelfare);
            }
            publicWelfareService.incrementImport(publicWelfareList);
    ...
    public ResponseRes<Object> coverImport(MultipartFile file) {
        try {
            ...
            publicWelfareService.coverImport(publicWelfareList);
        } catch (FileException e1) {
            e1.printStackTrace();
            ResponseRes.error(e1.getCode(),e1.getMsg());
        } catch (Exception e2){
    ...
    @ResponseBody
    public ResponseRes<Object> listPW(PageRequest pageRequest, HttpSession session){
        ...
        try {
            ...
            if(flag){
                pageResult = publicWelfareService.listPW(pageRequest, menter_Id);
            }
    ...
    @ResponseBody
    public ResponseRes<Object> searchPW(PageRequest pageRequest, HttpSession session,
        ...
        try {
            flag = menterService.isExist(menter_Id);
            if(flag){
                pageResult = publicWelfareService.searchPW(pageRequest, menter_Id, student, semester);
            }
    ...
    public ResponseRes<Object> downloadTmp(HttpServletResponse response){
        try {
            DownloadUtil.downloadTmp("publicwelfare",response);
